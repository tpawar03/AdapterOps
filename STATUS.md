...
and promoted in manifest v6 over v5, with the same-session regression run attached and nothing blocked. Two
numbers still describe the previous adapter and are not re-measured: the confidence router's PII escalation
rate (0% at the operating point) and PII's training spread behind its 0.0081 gate.
