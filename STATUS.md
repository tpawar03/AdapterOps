...
| **Laptop PII numbers hold on vLLM** | `pii-false-positives --base-url` and `request-path-run --direct-vllm --tasks pii`, manifest v11 on an A10 · false positives **4/928** (4 grounded, 1 invented) and **4/491** (1 grounded, 3 invented), identical to the laptop · PII routing: **0 escalations** in both populations (100 and 253 pairs), same decision as the curve on all, score Spearman vs recorded 0.73 / 0.79 at medians under 0.002 · served PII success 0.75 / 0.76 | Phase 5 |
| **Where PII's and drafting's P95 goes** | `adapterops latency-profile` (streamed straight to vLLM, 3 LoRAs) · first token p95 **25–28 ms** at concurrency 1 and **40–41 ms** at 16 on every adapter · decode **9.5–11.2 ms/token** · total p95 at c1 / c16: intent 100 / 129 ms · **PII 1,443 / 1,659 ms** (p50 57 tokens) · **drafting 2,741 / 3,264 ms** (p50 98–102 tokens) · the miss is reply length, not queueing or prefill: streaming meets 500 ms for drafting's first token, PII needs a full-reply target | Phase 5 |
| **§7 latency is per task (PRD v2.16, changelog 67)** | Set on where the time goes, not to fit the total: intent and urgency < 500 ms full reply; drafting < 500 ms to first token (40 ms); PII < 2 s full reply (1,659 ms), chosen after that measurement and stated as such · the single-target misses (2,259 and 3,000 ms) stay recorded · dashboard §7 reads both | Phase 5 |
| **GPT-4o-mini under sustained load, domain gate on** | 30 min at concurrency 32, budget 300/min and 7,000/day · **68,691 pairs, 0 HTTP errors**, fallback 0.25% · 34–35 pairs/s while the frontier answered (300/min binding every minute), **43/s** after it stopped · client p95 3.4–3.8 s, then 2.4 s · frontier p95 6.0 s · escalation 16.9%, **out of domain 3.3%** (drafting 7.8%, urgency 3.0%, intent 2.2%, PII 0) · graded served success 0.725 vs curve 0.710 · 97.0% same decision as the curve · $1.02 · **bug found:** the frontier's 5,000-request lifetime cap stopped answers at minute 17, before the 7,000 budget, and 1,999 budget slots went to calls it refused; the cap now follows `--frontier-per-day` | Phase 5 |
